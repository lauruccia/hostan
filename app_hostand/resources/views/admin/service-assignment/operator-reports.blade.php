                ...
                <div class="card-body">
                    <form method="GET" action="{{ route('admin.service-assignment.operator-reports') }}" class="row">
                        ...
                        <div class="col-md-2 mb-3 d-flex align-items-end">
                            <button type="submit" class="btn btn-primary me-2" title="{{ __('Search') }}">
                                <i class="ti ti-search"></i>
                            </button>
                            @if(isset($operatorId) && $operatorId)
                                <a href="{{ route('admin.service-assignment.operator-reports.export', ['operator_id' => $operatorId, 'start_date' => $startDate ?? '', 'end_date' => $endDate ?? '']) }}" 
                                   class="btn btn-danger me-2" title="{{ __('Download PDF') }}">
                                    <i class="ti ti-file-text"></i>
                                </a>
                            @endif
                            <a href="{{ route('admin.service-assignment.operator-reports') }}" class="btn btn-outline-secondary" title="{{ __('Reset') }}">
                    ...
                    <div class="card-body">
                        <div class="tab-content" id="reportTabsContent">
                            ...
                            <div class="tab-pane fade show active" id="hourly" role="tabpanel">
                                ...
                                @if(isset($todayServicesForHourly) && $todayServicesForHourly->count() > 0)
                                    <div class="table-responsive">
                                        <table class="table table-hover">
                                            ...
                                            <tbody>
                                                @foreach($todayServicesForHourly as $service)
                                                    @php
                                                        $settings = settings();
                                                        $timezone = !empty($settings['timezone']) && $settings['timezone'] !== '' ? $settings['timezone'] : 'UTC';
                                                        $serviceDate = !empty($service->arrival_time) 
                                                            ? \Carbon\Carbon::parse($service->arrival_time, 'UTC')->setTimezone($timezone)
                                                            : null;
                                                    ...
                                                    <tr>
                                                        <td>
                                                            @if($serviceDate)
                                                                <strong>{{ $serviceDate->format('M d, Y') }}</strong>
                                                            @else
                                ...
                                @else
                                    <div class="text-center py-4">
                                        ...
                                        <p class="text-muted">{{ __('No services found for today\'s hourly breakdown.') }}</p>
                                    </div>
                                @endif
                            </div>
                            ...
                            <div class="tab-pane fade" id="daily" role="tabpanel">
                                ...
                                @else
                                    <div class="text-center py-4">
                                        ...
                                        <p class="text-muted">{{ __('No services found for this week\'s daily breakdown.') }}</p>
                                    </div>
                                @endif
                            </div>
