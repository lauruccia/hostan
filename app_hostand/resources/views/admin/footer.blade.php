...
<script src="{{ asset('vendor/js/select2/select2.js') }}"></script>
<script>
    // --- SAFE CHECK FOR SELECT2 ---
    // If select2.js is missing (404), this prevents other scripts from crashing.
    if (typeof jQuery !== 'undefined' && !$.fn.select2) {
        $.fn.select2 = function() {
            console.warn('Select2 is missing! Using fallback to prevent crashes. Please upload: public/vendor/js/select2/select2.js');
            return this;
        };
    }
</script>
...
<script>
    ...
    function initLightbox() {
        if (typeof GLightbox !== 'undefined') {
            ...
            console.log('GLightbox initialized. Elements found:', document.querySelectorAll('.glightbox').length);
        }
    }
    ...
    // 2. Re-initialize when AJAX content loads
    $(document).ajaxComplete(function() {
        setTimeout(initLightbox, 300); // Small delay to ensure DOM is ready
    });

    // 3. MutationObserver (Best for modals)
    // This watches for new elements added to the body
    const observer = new MutationObserver((mutations) => {
        let shouldInit = false;
        mutations.forEach((mutation) => {
            if (mutation.addedNodes.length) {
                mutation.addedNodes.forEach((node) => {
                    if (node.nodeType === 1 && (node.classList.contains('glightbox') || node.querySelector('.glightbox'))) {
                        shouldInit = true;
                    }
                });
            }
