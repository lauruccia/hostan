                ...
                <div class="card border-0 shadow-sm">
                    ...
                    <div class="card-body py-3">
                        <div class="d-flex flex-wrap gap-2">
                            @if ($hasThumbnail)
                            <img src="{{ asset('uploads/property/' . $propForImages->thumbnail->image) }}" alt="{{ __('Property photo') }}"
                                class="img-thumbnail rounded" style="width:120px; height:auto; object-fit:cover;">
                            @endif
                            @foreach ($extraImages as $image)
                            @if (!empty(trim($image->image ?? '')))
                            <img src="{{ asset('uploads/property/' . $image->image) }}" alt="{{ __('Property photo') }}"
                                class="img-thumbnail rounded" style="width:120px; height:auto; object-fit:cover;">
                            @endif
                            @endforeach
                        </div>
                    ...
                    @foreach ($completionImages as $imagePath)
                        @if (!empty($imagePath))
                            @php
                                ...
                                // Use same approach as property images - extract filename and prepend folder
                                $imagePath = trim($imagePath);
                                $imagePath = ltrim($imagePath, '/');
                                
                                // If path already contains uploads/facility_images/, use it directly
                                // Otherwise extract filename and build path
                                if (strpos($imagePath, 'uploads/facility_images/') === 0) {
                                    // Path is already correct: "uploads/facility_images/filename.jfif"
                                    ...
                                    // Extract just filename and build path (same as property images)
                                    $filename = basename($imagePath);
                                    $imageUrl = asset('uploads/facility_images/' . $filename);
                                }
                            @endphp
                            <a href="{{ $imageUrl }}" target="_blank" class="position-relative">
                                <img src="{{ $imageUrl }}" alt="Facility Photo"
                                    class="img-thumbnail rounded" style="width:120px; height:120px; object-fit:cover; cursor:pointer;">
                            </a>
                        @endif
                    @endforeach
                        </div>
